...
| | executed actions with adherence below threshold | 3 | 0 |
...
| | videoTimestampMs, when a screenshot exists | 0/9 | 9/9 |
| | rootCause | 4/13 | every class that can state one honestly |
| | grounding | 1/13 | ≥ half |
| contract | §30 sections met / partial / missing | 6 / 2 / 1 (§9 overstated) | 9 / 0 / 0, computed rather than rated by hand |
...
| **JRN-3** | Make the adherence gate a gate. After its attempts are used up with a score below threshold, pick a coping action, not the rejected action. Give the judge the target's label and box, not only its ref. Then cut its cost: skip it when the action matches the persona's stated plan, or give it a cheaper per-role model through `/webui`. | 2d (3, 4, 0 executed; `e367` judged "unrelated" while it was Pricing); 2i (47% of model wall) | Executed actions below threshold = 0. No objection names a bare ref. Adherence share ≤ 25%. |
...
| **SEC-5** | Video timestamps (§30.5). Set `videoTimestampMs` and `step` from the capture event's own `videoTimeMs`. | 2f: 0/9, though the data is present | 9/9 filled. |
