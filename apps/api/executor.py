...
class JobExecutor(ReportAssembler):
    ...
    def _combined_test(self, job: dict[str, Any]) -> dict[str, Any]:
        ...
        # RUN-0: dispatch is done; everything from here on reads only
        # journeys/personas/tasks, plus the small set of resolved values below,
        # so it lives in ReportAssembler.assemble_report -- the same method
        # RUN-3's offline replay calls with no live worker and no job/store at
        # all. `vision`/`redesign` are the same provider chain (both were
        # `self._providers_for(job, ROLE_VISION)` before the move); resolved
        # once, here, since only this method may read `job`.
        vision_providers = self._providers_for(job, ROLE_VISION)
        return self.assemble_report(
            url=data.get("url"), tasks=tasks, personas=personas,
            persona_artifacts=persona_artifacts, journeys=journeys,
            worker_configured=bool(worker_url), job_id=job["job_id"],
            vision=vision_providers, redesign=vision_providers,
            send_vision_options=bool(providers_for(job.get("workspace_id") or "local",
    ...
    def _dispatch_persona_run(self, job: dict[str, Any], data: dict[str, Any], tasks: list[str],
        ...
        payload = json.dumps({"runId": run_id, "url": data.get("url"),
            ...
            **({"identity": run_identity} if run_identity else {}),
            ...
            # worker side.
            **({"redactSelectors": redact_selectors} if redact_selectors else {}),
            # CAP-2: this run's hat's extra faculties, resolved from the hat
